...
// NewPackageRequest contains information about a new package.
type NewPackageRequest struct {
	Name string `json:"name"`
	Description string `json:"desc"`
	Labels []string `json:"labels"`
	Licenses []string `json:"licenses"`
	URL string `json:"vcs_url"`
	PublicDownloads bool `json:"public_download_numbers"`
}

// NewClient returns a new Client API client. If a nil httpClient is
// provided, http.DefaultClient will be used.
func NewClient(httpClient *http.Client, subject, username, apikey string) *Client {
	if httpClient == nil {
		httpClient = http.DefaultClient
	}
	baseURL, _ := url.Parse(defaultBaseURL)
	c := &Client{client: httpClient, BaseURL: baseURL, UserAgent: userAgent, subject: subject,
...
func (c *Client) CreatePackage(subject, repository string, request *NewPackageRequest) error {
	if "" == request.Description {
		request.Description = fmt.Sprintf("%s automatically generated by go-bintray", request.Name)
	}
	...
	if "" == subject || "" == request.Name || 0 == len(request.Licenses) || "" == request.URL {
		return errors.New("CreatePackage: required fields are empty")
	}
	...
	url := "/packages/" + subject + "/" + repository

	req, err := c.newRequestWithBody("POST", url, string(body))
	if err != nil{
		return err
	}
